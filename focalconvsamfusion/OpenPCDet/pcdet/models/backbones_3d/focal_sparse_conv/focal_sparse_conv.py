...
from pcdet.models.backbones_3d.focal_sparse_conv.utils import split_voxels, check_repeat, FocalLoss, fusion
from pcdet.models.backbones_3d.focal_sparse_conv.wavevit import Block as WaveBlock
from pcdet.models.backbones_3d.focal_sparse_conv.wavevit import Fastwaveblock
from pcdet.utils import common_utils

def GenerateDepths(depth_features, depth_mean=14.41, depth_var=156.89):
...
class FocalSparseConv(spconv.SparseModule):
    ...
    def __init__(self, inplanes, planes, voxel_stride, norm_fn=None, indice_key=None,
                ...
                voxel_size = [0.1, 0.05, 0.05], zgx_code=False):
        ...
        if use_img and (not zgx_code): 
            self.frequency_fuse = fusion(dim=16) # is dong code
    ...
    def construct_multimodal_features(self, x, x_rgb, batch_dict, fuse_sum=False):
        ...
        """
        # import pdb;pdb.set_trace() #!dxg
        #print('fuse_sam_before: ',self.fuse_sum)
        batch_index = x.indices[:, 0]
        spatial_indices = x.indices[:, 1:] * self.voxel_stride
        voxels_3d = spatial_indices * self.voxel_size + self.point_cloud_range[:3] # 体素坐标转换为点云坐标(z,y,x)
        # print("voxels_3d1", voxels_3d)
        calibs = batch_dict['calib']
        # depth_points = batch_dict['depth_points'] # [batch_size,x,y,z,r] liulin code
        # print('depth_points',depth_points) # [batch_size,x,y,z,r]
        ...
        if self.use_wave:
            B, N, H, W = x_rgb.shape
            x_rgb = x_rgb.view(B,N,H*W).permute(0,2,1)
            x_rgb = self.wave_block(x_rgb,H=H,W=W)
            x_rgb = x_rgb.view(B, H, W, N).permute(0,3,1,2)
            # print("x_rgb_batch after ",x_rgb.shape)
        '''
        ...
        for b in range(batch_size):
            # print('fuse_sam: ',fuse_sum)
            x_rgb_batch = x_rgb[b]
            calib = calibs[b]
            voxels_3d_batch = voxels_3d[batch_index==b]
            # print("voxels_3d_batch",voxels_3d_batch)
            ...
            if 'noise_scale' in batch_dict:
                ...
                # print("voxels_3d_batch[:, :3]",voxels_3d_batch[:, :3])
            if 'noise_rot' in batch_dict:
                ...
                # print("voxels_3d_batch[:, self.inv_idx]",voxels_3d_batch[:, self.inv_idx])
            ...
            # print("voxels_3d_batch max: ",voxels_3d_batch[:,0].max(),voxels_3d_batch[:,1].max(),voxels_3d_batch[:,2].max())
            voxels_2d, _ = calib.lidar_to_img(voxels_3d_batch[:, self.inv_idx].cpu().numpy()) # 投影到图像
            # print(voxels_2d)
            ...
            voxels_2d_int = voxels_2d_int[filter_idx] # 过滤掉图像外的点
            ...
            image_features_batch[filter_idx] = x_rgb_batch[:, voxels_2d_int[:, 1], voxels_2d_int[:, 0]].permute(1, 0).contiguous() # 取出被索引的像素特征

            if fuse_sum and self.zgx_code: # zgx code
                image_with_voxelfeature = image_features_batch + voxel_features_sparse
                #print("a")
            elif fuse_sum and not(self.zgx_code): # 融合方式 dong code
                image_with_voxelfeature = self.frequency_fuse(image_features_batch.unsqueeze(0) + voxel_features_sparse.unsqueeze(0)).squeeze()
                #print("b")
            else:
                image_with_voxelfeature = torch.cat([image_features_batch, voxel_features_sparse], dim=1)
                # print(fuse_sum,self.zgx_code)
                #print("c")
            #print("image_with_voxelfeature: ",image_with_voxelfeature.shape)
    ...
    def forward(self, x, batch_dict, x_rgb=None):
        ...
        if self.use_img:
            ...
            # print("features_multimodal: ",features_multimodal.shape)
