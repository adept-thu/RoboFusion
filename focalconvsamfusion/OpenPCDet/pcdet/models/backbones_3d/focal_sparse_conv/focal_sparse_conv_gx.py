...
class FocalSparseConv(spconv.SparseModule):
    ...
    def construct_multimodal_features(self, x, x_rgb, batch_dict, fuse_sum=False):
        ...
        voxels_3d = spatial_indices * self.voxel_size + self.point_cloud_range[:3] # 体素坐标转换为点云坐标
        ...
        if not x_rgb.shape == batch_dict['images'].shape:
            # np.save(f"{featmap_path}{batch_dict['frame_id'][0]}_16x256x256.npy", x_rgb.cpu().numpy().squeeze(0))
            x_rgb = nn.functional.interpolate(x_rgb, (h, w), mode='bilinear')
            # np.save(f"{featmap_path}{batch_dict['frame_id'][0]}_16x375x1242.npy", x_rgb.cpu().numpy().squeeze(0))
