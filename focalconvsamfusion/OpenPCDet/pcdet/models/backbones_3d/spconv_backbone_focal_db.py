    ...
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        if use_img:
            if use_seg:
                ...
                self.reduce_block = BasicBlock2D(**channel_reduce)
            elif use_sam_onlinev2: # v2 and v3 msfeat
                self.sam_encoder = SamImageEncoder(encoder_checkpoint=img_pretrain, is_optimize=False)
                ...
                self.sam_encoder = MobileSamImageEncoder(encoder_checkpoint=img_pretrain, is_optimize=False)
                channel_reduce={
                    "in_channels": 256,
                    "out_channels": 16,
                    "kernel_size": 1,
            ...
            self.conv_focal_multimodal = FocalSparseConv(16, 16, image_channel=model_cfg_seg['channel_reduce']['out_channels'][0] if use_seg else channel_reduce['out_channels'],
                                        ...
                                        voxel_stride=1, norm_fn=norm_fn, indice_key='spconv_focal_multimodal',zgx_code=zgx_code)
    ...
    def forward(self, batch_dict):
        ...
        if self.use_img:
            if self.use_seg:
                ...
                x_image = self.reduce_block(x_image) # channel 256-->16
            elif self.use_sam_onlinev2: # v2 and v3 msfeat
                x_image = batch_dict['images']
                x_image = self.sam_encoder(x_image)
                x_image = self.sam_upsample(x_image)
