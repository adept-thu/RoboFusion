...
def flare_source_gaussian(image, point, radius, src_color, finetune_ratio, alpha=1):
    ...
    canvas = image.copy().astype(np.float_) # modified by dxg: numpy 1.20 np.float->np.float_
    xy_center = point
    sigma = radius
    imgw, imgh = canvas.shape[1], canvas.shape[0]
    xc, yc = xy_center[0], xy_center[1]
    x = np.linspace(-xc, imgw-xc-1, imgw).astype(np.int_) # modified by dxg, numpy 1.20 np.int->np.int_
    y = np.linspace(-yc, imgh-yc-1, imgh).astype(np.int_) # modified by dxg, numpy 1.20 np.int->np.int_
