...
from .torch_wavelet import DWT_2D, IDWT_2D
...
class WaveAttention1(nn.Module):
    def __init__(self, dim, num_heads, sr_ratio):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
    ...
    def forward(self, x, H, W):
        ...
        x_idwt = self.idwt(x_dwt)
        x_idwt = x_idwt.view(B, -1, x_idwt.size(-2)*x_idwt.size(-1)).transpose(1, 2)
        #print("x_dwt",x_idwt.shape)

        kv = self.kv_embed(x_dwt).reshape(B, C, -1).permute(0, 2, 1)
        kv = self.kv(kv).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        #print("x_dwt",kv.shape)
        k, v = kv[0], kv[1]
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        #print("1",x.shape)
        #print("2",x_idwt.shape)
        x = self.proj(torch.cat([x, x_idwt], dim=-1))
        #print("1",x.shape)
        return x

@BACKBONES.register_module()
class filterfastsam(nn.Module):
    def __init__(self,indim=320,outdim=320):
        super().__init__()
        self.block1 = nn.Sequential(
            nn.Conv2d(indim, outdim, kernel_size=3, padding=1, stride=2),
...
class Fastwaveblock(nn.Module):
    ...
    def forward(self,x,H,W):
        ...
        x = x.view(B, H, W, C).permute(0, 3, 1, 2)
        #print(x.shape) # 1*16*64*64
        x_dwt = self.reduce(x)
        #print("x_dwt0",x_dwt.shape)
        x_dwt = self.dwt(self.reduce(x))
        #print("x_dwt1",x_dwt.shape)
        x_dwt = self.filter(x_dwt)
        #print("x_dwt2",x_dwt.shape)
        x_idwt = self.idwt(x_dwt)
        #print("x_idwt3",x_idwt.shape)
        x_idwt = x_idwt.view(B, -1, x_idwt.size(-2)*x_idwt.size(-1)).transpose(1, 2)
        #print("x_idwt4",x_idwt.shape)
        x_idwt = self.proj(x_idwt)
        #print("x_idwt5",x_idwt.shape)
        return x_idwt
'''
block = Fastwaveblock(dim=16)
demo_in = torch.randn(1,64*64,16)
demo_in = block(demo_in,64,64)
'''
@BACKBONES.register_module()
class Depthfilter(nn.Module):
    def __init__(self,indim=2,outdim=16):
        super().__init__()
        self.block1 = nn.Sequential(
            nn.Conv2d(indim, 4*indim, kernel_size=3, padding=0, stride=2),
            nn.BatchNorm2d(4*indim),
            nn.ReLU(inplace=True),
        )
        self.maxpool1 = nn.MaxPool2d(kernel_size=4, stride=2, padding=2)
        self.block2 = nn.Sequential(
            nn.Conv2d(4*indim, 8*indim, kernel_size=3, padding=0, stride=2),
            nn.BatchNorm2d(8*indim),
            nn.ReLU(inplace=True),
        )
        self.maxpool2 = nn.MaxPool2d(kernel_size=4, stride=2, padding=2)
...
class Depthimgfusion(nn.Module):
    def __init__(self,dim):
        super().__init__()
...
'''
blocktest = Depthfilter(2,16)
datademo = torch.randn(1,2,1024,1024)

output = blocktest(datademo)
print(output.shape)
'''